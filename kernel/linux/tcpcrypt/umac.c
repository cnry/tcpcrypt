...
static void uhash_init(uhash_ctx_t ahc, aes_int_key prf_key)
{
    ...
    for (i = 0; i < STREAMS*4; i++)
        ahc->ip_keys[i] = do_div(ahc->ip_keys[i], p36);  /* Bring into Z_p36 */
...
/* Allocate memory and force to a 16-byte boundary. */
    ...
    ctx = (uhash_ctx_t)kmalloc(sizeof(uhash_ctx)+ALLOC_BOUNDARY, GFP_ATOMIC);
    if (ctx) {
        if (ALLOC_BOUNDARY) {
 ...
 */
{
    ...
    ctx = (umac_ctx_t)kmalloc(sizeof(umac_ctx)+ALLOC_BOUNDARY, GFP_ATOMIC);
    if (ctx) {
        if (ALLOC_BOUNDARY) {
            bytes_to_add = ALLOC_BOUNDARY -
                              ((ptrdiff_t)ctx & (ALLOC_BOUNDARY - 1));
